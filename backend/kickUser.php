...
if ($teamInfo['position'] !== 'leader') {
    onError($conn, "You are not the Leader of this team!");
}

$kickUserId = getPostParam('userid');

if (empty($kickUserId)) {
    onError($conn, "User ID is required.");
}

$sql = "SELECT * FROM `teamates` WHERE `user_id` = ? AND `team_id` = ?";
$result = fetchDataFromQuery($conn, $sql, [$kickUserId, $teamInfo['teamid']], 'ii', "Failed to fetch user information");
if (empty($result)) {
    onError($conn, "User is not in your team.");
}

$sql = "DELETE FROM `teamates` WHERE `user_id` = ? AND `team_id` = ?";
executeQuery($conn, $sql, [$kickUserId, $teamInfo['teamid']], 'ii',false,"Failed to remove user from team.");
deleteTeamIfEmpty($conn, $teamInfo['teamid']);

onSuccess($conn, "User removed from team successfully.");
